...
module.exports = async (
  ...
  const transformedAST = await mapChildren(markdownAST, async (node) => {
      if (node.type !== 'link') {
          return node;
      }
      if (node.url.startsWith(EMBED_PROTOCOL)) {
        const sandboxId = generateSandbox(getDirectoryPath(node.url, EMBED_PROTOCOL));
        return {
          type: 'html',
          value: getIframe(`https://codesandbox.io/embed/${sandboxId}`)
        }
      }
      if (node.url.startsWith(LINK_PROTOCOL)) {
        const sandboxId = generateSandbox(getDirectoryPath(node.url, LINK_PROTOCOL));
